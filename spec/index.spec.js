...
describe("Test Index", function () {
    ...
    it('should retrieve indexes', function () {
        ...
        return meta.createIndex(personType.name, index).then(function () {
            ...
            var index = indexes.filter(function (el) {
                return el.keys[0].name
            })[0];
            expect(index.isCompound).be.false;
    ...
    it('should create text index', function () {
        var index = new DB.metamodel.DbIndex([
            {name: DB.metamodel.DbIndex.TEXT}
        ]);

        expect(index.isTextIndex).be.true;
        expect(index.hasKey("name")).be.true;
        ...
        return meta.createIndex(personType.name, index).then(function () {
            ...
            expect(index.isTextIndex).be.true;
            expect(index.keys[0].name).eqls(DB.metamodel.DbIndex.TEXT);
        });
    });

    it('should drop text index', function () {
        var index = new DB.metamodel.DbIndex([
            {name: DB.metamodel.DbIndex.TEXT}
        ]);
    ...
    it('should create text index with options', function () {
        var index = new DB.metamodel.DbIndex([
            {name: DB.metamodel.DbIndex.TEXT}
        ], [{default_language: "german"}, {weights: {name: 2}}]);
        ...
        return meta.createIndex(personType.name, index).then(function () {
            ...
            expect(index.isTextIndex).be.true;
            expect(index.keys[0].name).eqls(DB.metamodel.DbIndex.TEXT);
            expect(index.options[0].weights.name).eqls(2);
            expect(index.options[1].default_language).eqls("german");
        });
    });
