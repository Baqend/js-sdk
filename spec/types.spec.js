...
describe('Test GeoPoint', function() {
  it("should construct with an latitude and longitude argument", function() {
    var point = new baqend.GeoPoint(56.5, 165.2);
  ...
  it("should construct with an array argument", function() {
    var point = new baqend.GeoPoint([-36.5, -92.3]);
  ...
  it("should construct with an geolike object argument", function() {
    var point = new baqend.GeoPoint({"latitude": 90, "longitude": -180.0});
  ...
  it("should construct from json argument", function() {
    var point1 = new baqend.GeoPoint({"latitude": -90, "longitude": 180.0});
    var point2 = new baqend.GeoPoint(point1.toJSON());

    expect(point1).eql(point2);
  });

  it("should compute distance", function() {
    var point1 = new baqend.GeoPoint(53.5753, 10.0153); // Hamburg
    var point2 = new baqend.GeoPoint(40.7143, -74.006); // New York
    var point3 = new baqend.GeoPoint(-33.8679, 151.207); // Sydney
    var point4 = new baqend.GeoPoint(51.5085, -0.1257); // London
...
describe('Test entity type', function () {
  ...
  var EntityType = new baqend.metamodel.EntityType('jstest.Type', metamodel.entity(Object));
  var EmbeddedType = new baqend.metamodel.EmbeddableType('jstest.Embedded');
  var simpleTypes = {
    ...
    "geoPoint": { type: metamodel.baseType('GeoPoint'), values: [new baqend.GeoPoint(34.5658, 110.4576)] }
  };
  ...
  beforeEach(function() {
    obj = EntityClass();
    state = baqend.util.Metadata.get(obj);
  });
  ...
  describe("geoPoint value", function() {
    test("geoPoint", new baqend.GeoPoint(), new baqend.GeoPoint(34.5658, 110.4576), new baqend.GeoPoint(-45.67, -177.45));
  });
  ...
  describe("ref value", function() {
    ...
    it("should convert references to ids", function() {
      ...
      var refId = baqend.util.Metadata.get(ref).ref;
  ...
  describe("refList value", function() {
    test("refList", new baqend.List(), baqend.List([testObject(), testObject(), null, testObject()]));
  });

  describe("embeddedList value", function() {
    test("embeddedList", new baqend.List(), baqend.List([embeddableObject(), embeddableObject(), null, embeddableObject()]));
  });

  for (name in simpleTypes) {
    describe(name + "Set value", function() {
      var set = new baqend.Set();
      var vals = simpleTypes[name].values;
      for (var i = 0; i < vals.length; ++i) {
        set.add(vals[i]);
      }
  ...
  describe("refSet value", function() {
    test("refSet", new baqend.Set(), baqend.Set([testObject(), null, testObject(), testObject()]));
  });
  ...
  describe("simpleMap value", function() {
    ...
    test("simplebooleanMap", new baqend.Map(), new baqend.Map(map));

    it("should add key", function() {
      obj.simplebooleanMap = new baqend.Map(map);
  ...
  describe("simpleRefMap value", function() {
    ...
    test("simpleRefMap", new baqend.Map(), map);
  });
  ...
  describe("refSimpleMap value", function() {
    ...
    test("refSimpleMap", new baqend.Map(), map);
  });
  ...
  describe("refMap value", function() {
    ...
    test("refMap", new baqend.Map(), map);
  });
  ...
  describe("simpleEmbeddedMap value", function() {
    ...
    test("simpleEmbeddedMap", new baqend.Map(), map);
  });
