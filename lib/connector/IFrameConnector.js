...
exports.IFrameConnector = IFrameConnector = Connector.inherit(/** @lends jspa.connector.IFrameConnector.prototype */ {
  ...
  doReceive: function(event) {
    var cidString = this.cid + ':';
    if(event.origin !== this.origin || event.data.substring(0, cidString.length) !== cidString) {
      return;
    }

    var msg = JSON.parse(event.data.substring(cidString.length));
