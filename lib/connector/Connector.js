...
class Connector {
  ...
  static create(host, port, secure, basePath) {
    ...
    if (!connection) {
      ...
      for (let i = this.connectors.length - 1; i >= 0; i -= 1) {
        ...
        if (ConnectorConstructor.isUsable && ConnectorConstructor.isUsable(host, port, secure, basePath)) {
          connection = new ConnectorConstructor(host, port, secure, basePath);
          break;
        }
      }
  ...
  prepareResponse(message, response) {
    ...
    return new Promise((resolve) => {
      ...
      response.resultEntity = resultEntity;
