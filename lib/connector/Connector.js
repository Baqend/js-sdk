...
class Connector {
  ...
  prepareRequestEntity(message) {
    ...
    if (type) {
      ...
      let mediaType = message.header('content-type');

      switch (type) {
        case 'blob':
          mediaType = mediaType || entity.type;
          break;
        case 'arraybuffer':
          break;
        case 'data-url':
        ...
        case 'base64':
          ...
          type = 'blob';
          entity = new Blob([array], {type: mediaType});
          break;
        case 'json':
    ...
    if (!message.header('accept')) {
      message.header('accept', accept);
    }

    if (this.gzip) {
      let ifNoneMatch = message.header('if-none-match');
      if (ifNoneMatch && ifNoneMatch !== '""' && ifNoneMatch != '*') {
        message.header('if-none-match', ifNoneMatch.slice(0,-1) + '--gzip"');
      }
    }
