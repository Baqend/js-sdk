...
class EntityManager extends util.Lockable {
  ...
  ensureCacheHeader(entity, message, refresh) {
    ...
    if (refresh || cacheBypass() || isStale()) {
      if (!this.isChrome()) {
        message.setIfMatch('*'); // is needed for firefox or safari (but forbidden for chrome)
        message.setIfNoneMatch(''); // is needed for edge and ie (but forbidden for chrome)
        // message.request.headers['if-unmodified-since'] = new Date(0).toUTCString(); // sufficent for safari, not for firefox (and forbidden for chrome)
      }
      // message.request.headers['pragma'] = 'no-cache'; // sufficient for ie 10
   ...
   */
  isChrome() {
    return typeof window != 'undefined' && ((!!window.chrome && /google/i.test(navigator.vendor)) || (/cros i686/i.test(window.navigator.platform)));
  }
}
