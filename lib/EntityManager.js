...
class EntityManager extends util.Lockable {
  ...
  connected(connector, connectData, tokenStorage) {
    ...
    this.createObjectFactory(this.metamodel.embeddables);
    this.createObjectFactory(this.metamodel.entities);
    ...
    if (connectData) {
      ...
      if (connectData.user && tokenStorage.token) {
        this.updateUser(connectData.user, true);
      }
  ...
  send(mesage, ignoreCredentialError) {
    mesage.tokenStorage = this.tokenStorage;
    let result = this.connector.send(mesage);
  ...
  getReference(entityClass, key) {
    ...
    if (key) {
      ...
      id = entityClass;
      type = this.metamodel.entity(id.substring(0, id.indexOf('/', DB_PREFIX.length))); // skip /db/
    } else {
      type = this.metamodel.entity(entityClass);
    }
  ...
  insert(entity, options) {
    ...
    return this.store(entity, options, (state, json) => {
      if (state.version) {
  ...
  save(entity, options, withoutLock) {
    ...
    return withoutLock ? this.locklessStore(entity, options, msgFactory) : this.store(entity, options, msgFactory);
  }
   ...
   * @private
   */
  locklessStore(entity, options, msgFactory) {
    this.attach(entity);
    const state = Metadata.get(entity);
    let refPromises;
    ...
    this.getSubEntities(entity, options.depth).forEach((sub) => {
      refPromises.push(this.store(sub, subOptions, msgFactory));
    });
   ...
   * @private
   */
  store(entity, options, msgFactory) {
    ...
    return state.withLock(() => this.locklessStore(entity, options, msgFactory));
  }
  ...
  getSubEntities(entity, depth, resolved, initialEntity) {
    ...
    for (let value of state.type.references()) {
      this.getSubEntitiesByPath(entity, value.path).forEach((subEntity) => {
        if (resolved.indexOf(subEntity) === -1 && subEntity !== initialEntity) {
  ...
  register(user, password, loginOption) {
    ...
    return this.withLock(() => {
      ...
      return this.sendLogin(msg, loginOption);
    });
  }
  ...
  logout() {
    return this.withLock(() => {
      return this.send(new messages.Logout()).then(this.handleLogout.bind(this));
    });
  }
  ...
  loginWithOAuth(provider, clientID, options) {
    ...
    if (Message[provider + 'OAuth']) {
      ...
      msg.addRedirectOrigin(this.connector.origin + this.connector.basePath);
    } else {
      throw new Error('OAuth provider ' + provider + ' not supported.');
    }

    const windowOptions = { width: options.width, height: options.height };
    if (options.redirect) {
    ...
    const req = this.sendLogin(msg, options.loginOption);
    this.openOAuthWindow(msg.request.path, options.title, windowOptions);
  ...
  newPassword(username, password, newPassword) {
    return this.withLock(() => {
      ...
      return this.send(msg, true).then((response) => {
        return this.updateUser(response.entity);
      });
    });
  }
  ...
  newPasswordWithToken(token, newPassword, loginOption) {
    return this.withLock(() => (
      this.sendLogin(new messages.NewPassword({ token, newPassword }), loginOption)
    ));
  }
  ...
  sendLogin(msg, loginOption) {
    ...
    return this.send(msg, !login)
      .then(
        response => (response.entity ? this.updateUser(response.entity, login) : null),
        (e) => {
          if (e.status === StatusCode.OBJECT_NOT_FOUND) {
            if (login) {
              this.handleLogout();
  ...
  createURL(relativePath, authorize) {
    let path = this.connector.basePath + relativePath;
    ...
    return this.connector.origin + path;
  }
}
