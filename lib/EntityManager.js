...
class EntityManager extends util.Lockable {
    ...
    Object.keys(types).forEach((ref) => {
      ...
      if (this[name]) {
        ...
        Object.defineProperty(this, name, {
          value: type._createObjectFactory(this),
        });
      } else {
        Object.defineProperty(this, name, {
          get() {
  ...
  send(mesage, ignoreCredentialError) {
    ...
    let result = this.connector.send(msg);
  ...
  getSubEntities(entity, depth, resolved, initialEntity) {
    ...
    const state = Metadata.get(entity);
    for (let iter = state.type.references(), item = iter.next(); !item.done; item = iter.next()) {
    ...
    path.forEach((attributeName) => {
      ...
      subEntities.forEach((subEntity) => {
        ...
        if (attribute.isCollection) {
          for (let iter = curEntity.entries(), item = iter.next(); !item.done; item = iter.next()) {
...
deorecated(EntityManager.prototype, '_connector', 'connector');
