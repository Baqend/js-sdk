...
class EntityManager extends util.Lockable {
  ...
  /**
   * The observable network status of the window
  ...
  load(entityClass, oid, options) {
    ...
    if(this.offlineService.isOnline()) {
      ...
      return this.offlineService.loadLocalEntity(entity, state.type.name).then((loadedObject) => {
          state.setJson(loadedObject, true);
          return this.resolveDepth(entity, options);
      });
    }
  }
  ...
  _locklessSave(entity, options, msgFactory) {
    ...
    if(this.offlineService.isOnline()) {
      ...
      return this.offlineService.saveDataLocally(state.getJson(false, true), state.type.name, true).then(() => {
        return entity;
      });
    }
  ...
  'delete'(entity, options) {
    ...
    if(this.offlineService.isOnline()) {
        return state.withLock(() => {
            ...
            return Promise.all(refPromises).then(() => {
                this.offlineService.deleteLocalEntity(entity, state.type.name, false);
                return entity;
            });
        });
    } else {
        return this.offlineService.deleteLocalEntity(state.getJson(false,true), state.type.name, true).then(() => {
          return entity;
        });
    }
  }
