...
var EntityManager = Object.inherit(Lockable, /** @lends baqend.EntityManager.prototype */ {
  ...
  loginWithOAuth: function(provider, clientID) {
    if( provider == "google") {
      var src = 'https://accounts.google.com/o/oauth2/auth' +
          '?response_type=code' +
          '&client_id=' + clientID +
          '&redirect_uri=' + encodeURIComponent(this._connector.origin + '/db/User/OAuth/google') +
          '&scope=' + 'email' +
          '&state=' + encodeURIComponent('{"isGlobal" : "' + this.isGlobal + '"}') +
          '&access_type=' + 'online';

    } else if (provider == "facebook") {
      var src = 'https://www.facebook.com/dialog/oauth' +
          '?response_type=code' +
          '&client_id=' + clientID +
          '&scope=' + 'email' +
          '&state=' + encodeURIComponent('{"isGlobal" : "' + this.isGlobal + '"}') +
          '&redirect_uri=' + encodeURIComponent(this._connector.origin + '/db/User/OAuth/facebook');

    } else if (provider == "twitter") {
      prompt("twitter OAuth is not supported.")
      /*
      var http = new XMLHttpRequest();
      var url = 'https://api.twitter.com/oauth/request_token';
      http.setRequestHeader("Authorization", 'OAuth oauth_nonce="' + Math.floor(Math.random() * 100000000) + '", ' +
          'oauth_callback="' + encodeURIComponent('https://internal.baqend.com:8443/db/User/OAuth/facebook/?isGlobal=true') + '", ' +
          'oauth_signature_method="HMAC-SHA1", ' +
          'oauth_timestamp="' + Date.now() + '", ' +
          'oauth_consumer_key="' + 'ZBNHKepBC5qvuM5R6JuIM8iS1' + '", ' +
          'oauth_signature="' + Pc%2BMLdv028fxCErFyi8KXFM%2BddU%3D + '", ' +
          'oauth_version="1.0"';
      http.send()
          */
    } else if (provider == "github") {
      var src = 'https://github.com/login/oauth/authorize' +
          '?response_type=code' +
          '&client_id=' + clientID +
          '&redirect_uri=' + encodeURIComponent(this._connector.origin + '/db/User/OAuth/github') +
          '&scope=' + 'user:email' +
          '&state=' + encodeURIComponent('{"isGlobal" : "' + this.isGlobal + '"}') +
          '&access_type=' + 'online';
    }

    open(src, "OAuth", "width=500, height=600");
    return new Promise(function(resolve, reject) {
      window.addEventListener("storage", function (event) {
        if (event.key == 'storage-oauth-event') {
