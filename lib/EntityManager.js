...
class EntityManager extends util.Lockable {
  ...
  insert(entity, options) {
    ...
    return this._save(entity, options, (state, json) => {
      if (state.version)
  ...
  changeUsername(username, newUsername, password) {
    return this.send(new message.ChangeUsername({ username, newUsername, password }))
  }
  ...
  registerDevice(os, subscription, device) {
    ...
    return this.send(msg).then(response => {
      return this.updateDevice(response.entity);
    })
  }
  ...
  requestAPIToken(entityClass, user) {
    let userObj = this._getUserReference(entityClass, user);

    let msg = new message.UserToken(userObj.key);
  ...
  revokeAllTokens(entityClass, user) {
    ...
    let msg = new message.RevokeUserToken(userObj.key);
