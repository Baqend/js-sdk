...
class EntityManager extends util.Lockable {
  ...
  resolveDepth(entity, options) {
    ...
    if (!options || !options.depth) {
      if(offlineService.isOnline())
        offlineService.saveDataLocally(state.getJson(false, true), state.type.name);
      return Promise.resolve(entity);
    }
  ...
  load(entityClass, oid, options) {
    ...
    if(offlineService.isOnline()) {
      ...
      return offlineService.loadLocalEntry(entity, state.type.name).then((loadedObject) => {
          // refresh object if loaded older version from cache
          // chrome doesn't using cache when ifNoneMatch is set
  ...
  _locklessSave(entity, options, msgFactory) {
    ...
    if(offlineService.isOnline()) {
        ...
        return Promise.all(refPromises).then(() => {
            offlineService.saveDataLocally(state.getJson(false, true), state.type.name, false);
            return entity;
        });
    } else {
      return offlineService.saveDataLocally(state.getJson(false, true), state.type.name, true);
    }
    ...
    if(offlineService.isOnline()) {
        return state.withLock(() => {
            ...
            return Promise.all(refPromises).then(function() {
                offlineService.deleteLocalEntry(entity, state.type.name, false);
                return entity;
            });
        });
    } else {
        return offlineService.deleteLocalEntry(state.getJson(false,true), state.type.name, true);
    }
  }
