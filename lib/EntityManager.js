...
exports.EntityManager = EntityManager = util.QueueConnector.inherit(/** @lends jspa.EntityManager.prototype */ {
  ...
  initialize: function (entityManagerFactory) {
    this.superCall(null, entityManagerFactory.connector);
    this.entities = {};
  ...
  contains: function (entity) {
    ...
    var identifier = state.type.id.getDatabaseValue(state, entity);
  ...
  find: function (entityClass, oid, doneCallback, failCallback) {
    ...
    if (state.isLoaded) {
      ...
      promise = this.send(new message.GetObject(state, tid)).then(function () {
        if (state.isDeleted) {
          this.removeReference(entity);
          entity = null;
        }
