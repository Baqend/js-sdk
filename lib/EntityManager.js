 ...
 */
class EntityManager extends Lockable {
  /**
   ...
   */
  constructor(entityManagerFactory, global) {
    super();
  ...
  getReference(entityClass, id) {
    if (Object(entityClass) instanceof String) {
      if (!id) {
        // /db/class/id
    ...
    var entity = this._entities.get(type.ref + '/' + encodeURIComponent(id));
    if (!entity) {
      entity = type.create();
   ...
   */
  close(doneCallback, failCallback) {
    this._connector = null;
  ...
  contains(entity) {
    var metadata = Metadata.get(entity);
    return !!metadata && this._entities.get(metadata.ref) === entity;
  }
  ...
  resolveDepth(entity, options) {
    ...
    var subOptions = Object.assign({}, options, {
      dept: options.depth === true ? true : options.depth-1
    });
   ...
   * @return {Promise<baqend.binding.Entity>}
   */
  reload(entity, options) {
    options = options || {};
    options.reload = true;

    return this.load(entity.constructor, entity.id, options);
  }
  ...
  _attach(entity) {
    ...
    if (metadata.id) {
      this._entities.set(metadata.ref, entity);
    }
  }
  ...
  removeReference(entity) {
    ...
    this._entities.delete(state.ref);
  }

  register(user, password, login) {
    if (this.me && login)
