...
exports.EntityManager = EntityManager = Object.inherit(/** @lends jspa.EntityManager.prototype */ {
  ...
  _send: function(message) {
    message.withAuthorizationToken(this.isGlobal || this._token);
  ...
  register: function(username, password, globalSession) {
    if (this._me)
      throw new Error('User is already logged in.');

    globalSession = globalSession || this.isGlobal;
    ...
    msg.withAuthorizationToken(globalSession);

    return this._userRequest(msg);
  },

  login: function(username, password, globalSession) {
    if (this._me)
      throw new Error('User is already logged in.');
  ...
  renew: function(globalSession) {
    var msg = new message.RenewToken();
    msg.withAuthorizationToken(globalSession || this.isGlobal || this._token);
    return this._userRequest(msg);
  },
