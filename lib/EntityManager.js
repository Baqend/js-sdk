...
class EntityManager extends util.Lockable {
  ...
  send(message, withoutTokenstorage) {
    if (!withoutTokenstorage)
  ...
  newPassword(username, password, newPassword, loginOption) {
    return this.withLock(() => {
      const msg = new message.NewPassword({username, password, newPassword});
      return this._userRequest(msg, loginOption);
    });
  }
