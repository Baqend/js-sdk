...
var EntityManager = Object.inherit(Lockable, /** @lends baqend.EntityManager.prototype */ {
   ...
   */
  find: function(entityClass, oid, options) {
    options = options || {};
    ...
    return this._send(msg).then(function(msg) {
      ...
      var subPromise = [];
      if(options.depth) {
        options.depth = options.depth === true ? true : options.depth-1;
        this.getSubEntities(entity, 1).forEach(function(subEntity) {
          if(!~options.resolved.indexOf(subEntity)) {
            ...
            subPromise.push(this.find(subEntity.constructor, metadata.id, options));
          }
        }.bind(this));
      }
  ...
  _save: function(entity, options, msgFactory) {
    ...
    return state.withLock(function() {
      ...
      this.getSubEntities(entity, options.depth).forEach(function(sub) {
        options.depth = 0;
        refPromises.push(this._save(sub, options, msgFactory));
      }.bind(this));
  ...
  remove: function(entity, options) {
    ...
    return state.withLock(function() {
      ...
      this.getSubEntities(entity, options.depth).forEach(function(sub) {
        options.depth = 0;
        refPromises.push(this.remove(sub, options));
      }.bind(this));
  ...
  refresh: function(entity, options) {
    ...
    return this.find(entity.constructor, entity._metadata.id, options);
  },
