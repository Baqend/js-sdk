...
exports.EntityManager = EntityManager = Object.inherit(/** @lends jspa.EntityManager.prototype */ {
  ...
  find: function(entityClass, oid, doneCallback, failCallback) {
    ...
    if (state.isLoaded) {
      ...
      promise = this._connector.send(new message.GetObject(state.bucket, state.id)).then(function(message) {
  ...
  save: function(entity, force, depth, doneCallback, failCallback) {
    ...
    return entity.ready(function() {
      ...
      if (state.isDirty) {
        return this._connector.send(new message.ReplaceObject(state.bucket, state.id, state.getDatabaseObject())).then(function(msg) {
  ...
  remove: function(entity, force, depth, doneCallback, failCallback) {
    ...
    return entity.ready(function() {
      ...
      return this._connector.send(new message.DeleteObject(state.bucket, state.id, state.getDatabaseObjectInfo())).then(function() {
        this.removeReference(entity);
        state.setRemoved();
        return entity;
      }.bind(this));
