...
const OfflineService = require('../offline/offlineService');
...
class EntityManager extends util.Lockable {
  ...
  constructor(entityManagerFactory) {
     ...
     *
     */
    this.offlineService = new OfflineService(entityManagerFactory);
  }
  ...
  send(message, ignoreCredentialError) {
    message.tokenStorage = this.tokenStorage;
    let result = this._connector.send(message);
  ...
  resolveDepth(entity, options) {
    ...
    if (!options || !options.depth) {
      if(this.offlineService.isOnline())
        this.offlineService.saveDataLocally(state.getJson(false, true), state.type.name);
      return Promise.resolve(entity);
    }
  ...
  load(entityClass, oid, options) {
    ...
    const msg = new message.GetObject(state.bucket, state.key);
    ...
    if(this.offlineService.isOnline()) {
      ...
      return this.offlineService.loadLocalEntry(entity, state.type.name).then((loadedObject) => {
          // refresh object if loaded older version from cache
          // chrome doesn't using cache when ifNoneMatch is set
          if (entity.version > loadedObject.version) {
              options.refresh = true;
              return this.load(entityClass, oid, options)
          }

          this.addToWhiteList(loadedObject.id);
  ...
  save(entity, options, withoutLock) {
    ...
    const msgFactory = function(state, json) {
      if (options.force) {
        if (!state.id)
  ...
  _locklessSave(entity, options, msgFactory) {
    ...
    if(this.offlineService.isOnline()) {
        ...
        return Promise.all(refPromises).then(() => {
            this.offlineService.saveDataLocally(state.getJson(false, true), state.type.name, false);
            return entity;
        });
    } else {
      return this.offlineService.saveDataLocally(state.getJson(false, true), state.type.name, true);
    }
  ...
  getSubEntitiesByPath(entity, path) {
    ...
    path.forEach((attributeName) => {
      ...
      subEntities.forEach((subEntity) => {
        let curEntity = subEntity[attributeName];
        if (!curEntity)
          return;
  ...
  'delete'(entity, options) {
    ...
    if(this.offlineService.isOnline()) {
        return state.withLock(() => {
            ...
            const msg = new message.DeleteObject(state.bucket, state.key);
            ...
            return Promise.all(refPromises).then(function() {
                this.offlineService.deleteLocalEntry(entity, state.type.name, false);
                return entity;
            });
        });
    } else {
        return this.offlineService.deleteLocalEntry(state.getJson(false,true), state.type.name, true);
    }
  }
  ...
  attach(entity) {
    if (!this.contains(entity)) {
      let type = this.metamodel.entity(entity.constructor);
      if (!type)
        throw new error.IllegalEntityError(entity);
  ...
  register(user, password, loginOption) {
    ...
    return this.withLock(() => {
      let msg = new message.Register({ user, password, login });
  ...
  login(username, password, loginOption) {
    ...
    return this.withLock(() => {
      let msg = new message.Login({ username, password });
      return this._userRequest(msg, loginOption);
    });
  }
  ...
  loginWithOAuth(provider, clientID, options) {
    ...
    const req = this._userRequest(msg, options.loginOption);
    let w = open(msg.request.path, options.title, 'width=' + options.width + ',height=' + options.height);
  ...
  newPassword(username, password, newPassword) {
    return this.withLock(() => {
      var msg = new message.NewPassword({ username, password, newPassword });
  ...
  newPasswordWithToken(token, newPassword, loginOption) {
    return this.withLock(() => {
      return this._userRequest(new message.NewPassword({ token, newPassword }), loginOption);
    });
  }

  resetPassword(username) {
    return this.send(new message.ResetPassword({ username }));
  }
  ...
  _userRequest(msg, loginOption) {
    ...
    return this.send(msg, !login).then((response) => {
      if (response.entity) {
  ...
  registerDevice(os, token, device) {
    let msg = new message.DeviceRegister({
      token,
      device,
      devicetype: os
    });
  ...
  refreshBloomFilter() {
    ...
    const msg = new message.GetBloomFilter();
    msg.noCache();
    return this.send(msg).then((response) => {
  ...
  mustRevalidate(id) {
    ...
    let refresh = this.isCachingDisabled || !this._bloomFilterLock.isReady;
  ...
  createURL(relativePath, authorize) {
    let path = this._connector.basePath + relativePath;
