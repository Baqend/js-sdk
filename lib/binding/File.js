...
class File {
  ...
  /**
   * The MIME type of the file, only accessible after fetching the metadata or downloading/uploading/providing the file
   * @type {string}
   * @readonly
   */
  get mimeType() {
  ...
  /**
   * The encoding of the file, only accessible after fetching the metadata or downloading/uploading/providing the file
   * @type {binding.FileFactory.Encoding}
   * @readonly
   */
  get encoding() {
    if (this.isFolder) {
      throw new Error('A folder has no encoding');
    }
    this.checkAvailable();
  ...
  /**
   * Parses an "E-Tag" HTTP header header
   * @param {string=} eTag The "E-Tag" HTTP header to parse or something falsy
  ...
  /**
   * Parses a "Content-Encoding" HTTP header
   * @param {string=} contentEncoding The "Content-Encoding" HTTP header to parse or something falsy
   * @return {?binding.FileFactory.Encoding} Returns the parsed encoding or null, if it could not be parsed
   */
  static parseEncoding(contentEncoding) {
    if (!contentEncoding) {
      return null;
    }

    const toMatch = contentEncoding.trim().toLowerCase();
    const encodings = binding.FileFactory.Encoding;
    const value = Object.keys(encodings).find((key) => {
      const valueAtKey = encodings[key];
      return toMatch.indexOf(valueAtKey) !== -1;
    });

    return value ? encodings[value] : null;
  }
   ...
   * @return {Promise<?binding.File>} A promise which will be fulfilled with this file or null, if it does not exist
   */
  ...
  fromHeaders(headers) {
    ...
    this.fromJSON({
      ...
      encoding: File.parseEncoding(headers['content-encoding']),
  ...
  fromJSON(json) {
    if (json.id && this.id !== json.id) {
      throw new Error('This file id ' + this.id + ' does not match the given JSON\'s id ' + json.id);
    }
    ...
    this[METADATA] = Object.assign({}, this[METADATA], {
      ...
      encoding: json.encoding || binding.FileFactory.Encoding.IDENTITY,
