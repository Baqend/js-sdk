...
class File {
   ...
   * @param {object=} fileOptions.customHeaders The custom headers which will be send with the file after uploading it
   */
   ...
   * @param {object=} uploadOptions.customHeaders The custom headers which will be send with the file after uploading it
  ...
  upload(uploadOptions, doneCallback, failCallback) {
    ...
    if (this._customHeaders) {
      uploadMessage.customHeaders(this._customHeaders);
    }
  ...
  _fromHeaders(headers) {
    this.fromJSON({
      ...
      customHeaders: headers['baqend-custom-headers'] && JSON.parse(headers['baqend-custom-headers'])
    });
  }
  ...
  fromJSON(metadata) {
    ...
    if (metadata.customHeaders)
      this._customHeaders = metadata.customHeaders;
  ...
  toJSON() {
    ...
    if (this._customHeaders) {
      result.customHeaders = this._customHeaders
    }
    return result;
  }
