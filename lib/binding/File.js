...
class File {
  ...
  constructor(fileOptions) {
    ...
    if (Object(fileOptions) instanceof String) {
      ...
      if (id.indexOf('/file/') != 0 || nameSeparator == -1) {
      ...
      if (fileOptions.path) {
        ...
        if (parent.charAt(parent.length) != '/')
          parent = parent + '/';
   ...
   * @param {baqend.binding.File~fileCallback=} doneCallback The callback is invoked after the metadata is fetched
   * @param {baqend.binding.File~failCallback=} failCallback The callback is invoked if any error has occurred
