...
class File {
   ...
   * it
   * @param {connection.Message~progressCallback} [uploadOptions.progress] listen to progress changes during upload
  ...
  fromJSON(json) {
    ...
    if (json.mimeType) {
      meta.mimeType = json.mimeType;
    }

    if (json.lastModified) {
      meta.lastModified = new Date(json.lastModified);
    }

    if (json.createdAt) {
      meta.createdAt = new Date(json.createdAt);
    }
    ...
    if (json.size || json.contentLength) {
      meta.size = typeof json.size === 'number' ? json.size : json.contentLength;
    }

    if (json.acl instanceof Acl) {
      meta.acl = json.acl;
    } else {
      meta.acl = meta.acl || new Acl();
      if (json.acl) {
        meta.acl.fromJSON(json.acl);
      }
    }
  }
