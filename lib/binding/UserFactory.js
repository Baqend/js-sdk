...
var UserFactory = EntityFactory.extend( /** @lends binding.UserFactory.prototype */ {
  ...
  newPassword(username, password, newPassword, doneCallback, failCallback) {
    if (typeof newPassword !== 'string') {
      ...
      let arg = arguments;
      let token = arg[0], newPassword = arg[1], loginOption = arg[2], doneCallback = arg[3], failCallback = arg[4];
      ...
      return this._db.newPasswordWithToken(token, newPassword, loginOption).then(doneCallback, failCallback);
    } else {
   ...
   * @param {binding.Entity~failCallback=} failCallback Called when the operation failed.
   */
  revokeTokens(user, doneCallback, failCallback) {
    return this._db.revokeTokens(this._managedType.typeConstructor, user).then(doneCallback, failCallback);
  }
