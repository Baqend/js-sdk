...
/*
    options.width = options.width === null ? 585 : options.width;
options.height = options.height === null ? 545 : options.height;
msg = new Message[provider + 'OAuth'](clientID, scope, JSON.stringify(state));
} else if (provider == "facebook") {
  options.width = options.width === null ? 1140 : options.width;
  options.height = options.height === null ? 640 : options.height;
  scope = options.scope === null ? 'email' : options.scope;
  msg = new Message.FacebookOAuth(clientID, scope, JSON.stringify(state));
} else if (provider == "github") {
  options.width = options.width === null ? 1040 : options.width;
  options.height = options.height === null ? 580 : options.height;
  scope = options.scope === null ? 'user:email' : options.scope;
  msg = new Message.GitHubOAuth(clientID, scope, JSON.stringify(state));
} else if (provider == "twitter") {
  options.width = options.width === null ? 740 : options.width;
  options.height = options.height === null ? 730 : options.height;
  msg = new Message.TwitterOAuth();
} else if (provider == "linkedin") {
  options.width = options.width === null ? 630 : options.width;
  options.height = options.height === null ? 530 : options.height;
