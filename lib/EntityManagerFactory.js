...
class EntityManagerFactory extends util.Lockable {
  ...
  constructor(options) {
    ...
    if (!this.tokenStorage) {
      isReady = false;
      ready = ready.then(() => {
        return this.tokenStorageFactory.create(this.connector.origin);
      }).then((tokenStorage) => {
  ...
  createEntityManager(useSharedTokenStorage) {
    ...
    if (this.isReady) {
      em.connected(this.connector, this._connectData, useSharedTokenStorage? this.tokenStorage: new util.TokenStorage(this.connector.origin));
    } else {
      em.withLock(() => {
