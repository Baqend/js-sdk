...
class EntityManagerFactory extends util.Lockable {
  ...
  constructor(options) {
    ...
    if (!this.tokenStorage) {
      isReady = false;
      ready = ready
        .then(() => this.tokenStorageFactory.create(this.connector.origin))
        .then((tokenStorage) => {
  ...
  connect(hostOrApp, port, secure, basePath) {
    ...
    this.connector = Connector.create(hostOrApp, port, secure, basePath);
  ...
  createEntityManager(useSharedTokenStorage) {
    ...
    if (this.isReady) {
      em.connected(
        this.connector,
        this._connectData,
        useSharedTokenStorage ? this.tokenStorage : new util.TokenStorage(this.connector.origin)
      );
    } else {
  ...
  send(msg) {
    ...
    return this.connector.send(msg);
  }
}

deprecated(EntityManagerFactory.prototype, '_connector', 'connector');
