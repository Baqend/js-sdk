...
class Query {
  ...
  /**
   * Returns an observable that receives events for real-time query.
   ...
   * @return {Observable<StreamingEvent<T>>} an observable
   */
  eventStream(options) {}

  /**
   * Returns an stream subscription that receives events for real-time query.
   * @param {Object} [options] options on whether an initial result and what kind of events (w.r.t. match types and operations) are requested
   * @param {boolean} [options.initial=true] whether or not you want to receive the initial result set (i.e. the entities matching the query at subscription time)
   * @param {(string|Array<string>)} [options.matchTypes=['all']] the match types you are interested in; accepts the default ('all'/['all']) or any combination of 'match', 'add', 'change', 'changeIndex' and 'remove'
   * @param {(string|Array<string>)} [options.operations=['any']] the operations you are interested in; accepts the default ('any'/['any']) or any combination of 'insert', 'update', 'delete' and 'none'
   * @param {query.Query~nextEventCallback=} onNext Called when the next event received
   * @param {query.Query~failCallback=} onError Called when the operation failed.
   * @param {query.Query~completeCallback=} onComplete Called when the subscriptions completes through a network close event
  ...
  /**
   * Returns an observable that receives real-time query results.
   * @param {Object} [options] additional options
   * @return {Observable<Array<T>>} a real-time query subscription with an self maintaining query result
   */
  resultStream(options) {}

  /**
   * Returns an observable that receives real-time query results.
   * @param {query.Query~nextResultCallback=} onNext Called when any update on the query result happened
   * @param {query.Query~failCallback=} onError Called when the operation failed.
   * @param {query.Query~completeCallback=} onComplete Called when the subscriptions completes through a network close event
   * @return {Subscription} a real-time query subscription with an self maintaining query result
   * @name resultStream
   * @memberOf query.Query<T>.prototype
   * @method
   */
...
/**
 * This callback is invoked by each real-time query event
 * @callback query.Query~nextEventCallback
 * @param {StreamingEvent<T>} event The real-time event
 */

/**
 * This callback is invoked by each real-time query result update
 * @callback query.Query~nextResultCallback
 * @param {Array<T>} result The updated real-time query result
 */

/**
 * This callback is invoked when a real-time events completes by an inactivity timeout or network abortion
