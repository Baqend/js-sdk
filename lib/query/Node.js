...
class Node extends Query {
  ...
  resultList(options, doneCallback, failCallback) {
    ...
    const uriSize = this.entityManager.connector.host.length + query.length + sort.length;
    let msg;
  ...
  singleResult(options, doneCallback, failCallback) {
    ...
    const uriSize = this.entityManager.connector.host.length + query.length;
    let msg;
    if (uriSize > Query.MAX_URI_SIZE) {
  ...
  addOrder(fieldOrSort, order) {
    if (order) {
      this.sort[fieldOrSort] = order;
    } else {
      this.sort = fieldOrSort;
    }
    return this;
  }
