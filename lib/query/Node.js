...
class Node extends Query {
  ...
  resultList(options, doneCallback, failCallback) {
    ...
    if(this.entityManager.offlineService.isOnline()) {
        ...
        return this.entityManager.offlineService.queryLocalData(query, sort, this.maxResults, type.name).then((result) => {
            return this._createResultList(result, options);
        });
    }
  }
  ...
  singleResult(options, doneCallback, failCallback) {
    ...
    if(this.entityManager.offlineService.isOnline()) {
        ...
        return this.entityManager.offlineService.queryLocalData(query, sort, 1, type.name).then((result) => {
            return this._createResultList(result, options).then((list) => {
                return list.length ? list[0] : null;
            });
        });
    }
