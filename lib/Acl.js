...
class Acl {
  ...
  /**
   * Allows the given users or rules to read and write the object
  ...
  allowAccess(userOrRole) {
    const rules = Array.from(arguments);
    for (let i = 0; i < rules.length; i++) {
      this.allowReadAccess(rules[i]);
      this.allowWriteAccess(rules[i]);
    }
    return this;
  }

  /**
   * Denies the given users or rules to read and write the object
  ...
  denyAccess(userOrRole) {
    const rules = Array.from(arguments);
    for (let i = 0; i < rules.length; i++) {
      this.denyReadAccess(rules[i]);
      this.denyWriteAccess(rules[i]);
    }
    return this;
  }

  /**
   * Deletes the right for given rules to read and or write the object
   * @param {...(model.User|model.Role|string)} userOrRole The users or roles to delete for
   * @return {Acl} this acl object
   */
  deleteAccess(userOrRole) {
    const rules = Array.from(arguments);
    for (let i = 0; i < rules.length; i++) {
      this.deleteReadAccess(rules[i]);
      this.deleteWriteAccess(rules[i]);
    }
    return this;
  }
