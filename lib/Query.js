...
Query.Stream = Object.inherit(/** @lends baqend.Query.Stream.prototype */ {
  ...
  _wrapQueryCallback: function(cb) {
    return function(matchMessage) {
      //Create Object
      var operation = matchMessage.match.update.operation;
      var entity = null;
      if(operation != 'delete') {
        entity = this.entityManager.getReference(matchMessage.match.update.bucket, matchMessage.match.update.id);
        var metadata = Metadata.get(entity);
        metadata.setJson(matchMessage.match.update.object);
        metadata.setPersistent();
      }
      //Call wrapped callback
      cb(entity, operation, matchMessage.match.matchtype);
    }.bind(this);
  }
});
