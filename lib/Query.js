...
Query.Stream = Object.inherit(/** @lends baqend.Query.Stream.prototype */ {
  ...
  _wrapQueryCallback: function(cb) {
    ...
    return function(msg) {
      ...
      if(msg.match) {
        ...
        var entity = this._createObject(bucket, operation, msg.match.update.object);
        //Call wrapped callback
        cb(entity, operation, msg.match.matchtype);
      } else {
        //Initial result received
        if(!receivedResult) {
          console.log(msg);
          msg.result.forEach(function(object) {
            var operation = 'insert';
            var entity = this._createObject(bucket, operation, object);
            cb(entity, operation, 'match');
          }, this);
          receivedResult = true;
        }
      }
  ...
  _createObject: function(bucket, operation, object) {
    var entity = null;
    if(operation != 'delete') {
      entity = this.entityManager.getReference(bucket, object.id);
      var metadata = Metadata.get(entity);
