...
Query.Node = Object.inherit(Query, /** @lends baqend.Query.Node.prototype */ {
  ...
  stream: function(fetchQuery) {
    ...
    if (Object.getOwnPropertyNames(this._sort).length != 0) {
      throw new Error('No support for sorting, yet.');
    }

    if (fetchQuery === undefined)
      fetchQuery = true;

    return new Query.Stream(this.entityManager, type.name, this._serializeQuery(), fetchQuery);
  },
...
Query.Stream = Object.inherit(/** @lends baqend.Query.Stream.prototype */ {
  initialize: function(entityManager, bucket, query, fetchQuery) {
    this.entityManager = entityManager;
    this.bucket = bucket;
  ...
  on: function(event, callback) {
    var topic = [this.bucket, this.query, event, "any"].join("/");
    ...
    if(event === "add") {
      queryMessage.query.match = "match";
      queryMessage.query.operation = "insert";
    } else if (event == "remove") {
      queryMessage.query.match = "nonmatch";
      queryMessage.query.operation = "any";
    } else if (event == "change") {
      queryMessage.query.match = "match";
      queryMessage.query.operation = "update";
    }
  ...
  off: function(matchType, callback) {
    if (this.callbacks[matchType]) {
      var found = this.callbacks[matchType].filter(function(elem) {
        return callback ? elem.callback == callback : true;
      });

      found.forEach(function(entry) {
        this.entityManager._unsubscribe(entry.topic, entry.wrappedCallback);
        entry.queryMessage.register = false;
        this.entityManager._sendOverSocket(entry.queryMessage);
        this.callbacks[matchType].splice(this.callbacks[matchType].indexOf(entry), 1);
      }.bind(this));
    }
  },
  ...
  _wrapQueryCallback: function(cb) {
    return function(matchMessage) {
      //Create Object
      var entity = this.entityManager.getReference(matchMessage.match.update.bucket, matchMessage.match.update.id);
      var metadata = Metadata.get(entity);
      metadata.setJson(matchMessage.match.update.object);
      metadata.setPersistent();
      //Call wrapped callback
      cb(entity, matchMessage.match.update.operation, matchMessage.match.match);
    }.bind(this);
  }
});
