...
class ManagedType extends Type {
  ...
  *attributes() {
    if (this.superType) {
      for (let attribute of this.superType.attributes()) {
        yield attribute;
      }
    }
  ...
  *references() {
    for (let attribute of this.attributes()) {
      let type = attribute.isCollection ? attribute.elementType : attribute.type;
      if (type.isEntity) {
        yield {path: [attribute.name]};
      } else if (type.isEmbeddable) {
        for (let ref of type.references()) {
          yield {path: [attribute.name].concat(ref.path)};
        }
      }
    }
  }
