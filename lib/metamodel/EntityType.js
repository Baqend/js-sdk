...
class EntityType extends ManagedType {
  ...
  fromJsonValue (state, jsonObject, currentObject, isRoot) {
    if (isRoot || typeof jsonObject === 'object' && currentObject) {
  ...
  toJsonValue(state, object, options) {
    ...
    if (isInDepth && isAvailable) {
      var json = super.toJsonValue(state, object, Object.assign({}, options, {depth: options.depth - 1}) );
      if (!options.excludeMetadata)
        json = Object.assign(json, object._metadata.getJsonMetadata())
      return json;
