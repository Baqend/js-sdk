...
var State = require('../util/Metadata').Metadata;
...
exports.EntityType = EntityType = ManagedType.inherit( /** @lends jspa.metamodel.EntityType.prototype */ {
  ...
  toDatabaseValue: function (state, object) {
    if (state._root == object) {
      // root object
      var value = this.superCall(state, object);

      value._objectInfo = state.getDatabaseObjectInfo();

      return value;
    } else if (this.typeConstructor.isInstance(object)) {
      var id = this.id.getDatabaseValue(state, object);

      if (!id) {
        // attach new references
        object.attach(state.db);
        id = this.id.getDatabaseValue(state, object);
        if (!id) {
          throw new TypeError('Reference id is not available.');
        }
      }
...
EntityType.extend(/** @lends jspa.metamodel.EntityType */ {
  ...
  Object: EntityType.inherit(/** @lends jspa.metamodel.EntityType.Object.prototype */{
    ...
    initialize: function() {
      ...
      this.declaredId = new SingularAttribute(this, '_id', BasicType.String);
      this.declaredId.isId = true;
      this.declaredVersion = new SingularAttribute(this, '_version', BasicType.String);
