...
exports.Code = Code = Object.inherit(/** @lends baqend.metamodel.Metamodel.prototype */ {
  ...
  /**
   * Sets the function for the given bucket and handler type
   *
   * @param {String} bucket
  ...
  setHandler: function(bucket, codeType, fn) {
    ...
    this._handler[bucket][codeType] = String.isInstance(fn)? fn: this._toString(fn);
  },

  /**
   * Returns the function for the given bucket and handler type
   *
   * @param {String} bucket
   * @param {String} codeType (insert, update, delete)
   * @returns {Function}
  ...
  setCode: function(bucket, fn) {
    this._code[bucket] = String.isInstance(fn)? fn: this._toString(fn);
  },
  ...
  saveCode: function(bucket, fn, token) {
    ...
    return this._connector.send(msg).then(function(code) {
      this.setCode(bucket, code.response.entity);
   ...
   */
  callCode: function(bucket, obj, token) {
